...
@section('title')
    Edit Kategori - {{ $category->name }} - EvoKasir
@endsection
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-4xl">
        <div class="bg-gray-800/90 backdrop-blur-md p-4 sm:p-6 lg:p-8 rounded-xl shadow-lg animate-scale-in">
            <h2
                ...
                <i class="fas fa-chair text-lg sm:text-xl"></i> Edit - {{ $category->name }}
            </h2>
            ...
            <!-- Form -->
            <form action="{{ route('admin.categories.update', ['slug' => $slug, 'id' => $category->id]) }}" method="POST">
                @csrf
                @method('PUT')
