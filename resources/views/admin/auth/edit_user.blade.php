...
@section('title', 'Edit User - {{ $user->name }} - EvoKasir')
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-4xl">
        <div class="bg-gray-800/90 backdrop-blur-md p-4 sm:p-6 lg:p-8 rounded-xl shadow-lg animate-scale-in">
            <h2
                ...
                <i class="fas fa-user-edit text-lg sm:text-xl"></i> Edit User - {{ $user->name }}
            </h2>
            ...
            <form action="{{ route('admin.users.update', ['slug' => $slug, 'id' => $user->id]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4 sm:mb-6">
                    <label for="password" class="block text-sm sm:text-base font-semibold text-gray-300">Password (Kosongkan
