            ...
            <table class="w-full table-auto text-left text-sm sm:text-base">
                <thead class="bg-gray-700/50 border-b border-gray-600 text-gray-300">
                    <tr>
                        ...
                        <th class="py-3 sm:py-4 px-4 sm:px-6 font-semibold">Email</th>
                ...
                <tbody>
                    @forelse ($users as $user)
                        <tr class="border-b border-gray-700 hover:bg-gray-700/50 transition-all duration-200">
                            <td class="py-3 sm:py-4 px-4 sm:px-6 text-gray-300">{{ $user->name }}</td>
                            <td class="py-3 sm:py-4 px-4 sm:px-6 text-gray-300">{{ $user->email }}</td>
                            <td class="py-3 sm:py-4 px-4 sm:px-6 text-center">
                                <div class="flex justify-center gap-2 sm:gap-3">
                                    <a href="{{ route('admin.users.edit', ['slug' => $slug, 'id' => $user->id]) }}"
                                    ...
                                    <form action="{{ route('admin.users.destroy', ['slug' => $slug, 'id' => $user->id]) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="py-6 sm:py-8 text-center text-gray-400">Belum ada user.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
@push('scripts')
    <script>
        function confirmDelete(event) {
            ...
            Swal.fire({
                title: 'Hapus User?',
                text: 'User ini akan dihapus permanen. Lanjutkan?',
                icon: 'warning',
